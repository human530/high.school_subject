...
CURRICULUM.addBook({
  ...
  chapters: [
    /* ---------------------------------------------------------- 4-1 */
    {
      ...
      tags: ["空間坐標", "內積", "外積", "體積"],
      deep: [
        { h: "學測陷阱：別漏 z、別混內外積", b: "空間距離與長度公式記得加上 z 項(√(x²+y²+z²))。內積算出純量、外積算出向量，兩者用途不同別搞混。" },
        { h: "解題步驟：依目的選運算", b: "判垂直→內積=0；求『同時垂直兩向量』的方向或平行四邊形面積→外積 a×b；求平行六面體體積→三重積 |a·(b×c)|。" },
        { h: "技巧：平面≈空間只多一維", b: "平面向量的算法幾乎都能直接搬到空間，只是多一個 z 分量；先熟平面，空間就水到渠成。" }
      ],
      diagram: "space3d",
      fiveYO:
    ...
    /* ---------------------------------------------------------- 4-2 */
    {
      ...
      diagram: "plane",
      deep: [
        { h: "學測陷阱：係數就是法向量", b: "平面 ax+by+cz=d 的係數 (a,b,c) 就是法向量，一眼讀出。判兩平面平行看法向量平行、判垂直看法向量內積=0。" },
        { h: "解題步驟：先求法向量", b: "求平面方程式：拿平面上兩方向向量做外積得法向量→再代一個已知點定出常數項。點到平面距離與點到直線同款，只是多一項 z。" },
        { h: "技巧：夾角與交線", b: "兩平面夾角＝兩法向量的夾角(取銳角)。三元一次方程組＝三平面交點，可用消去法或行列式(克拉瑪公式)一次解出。" }
      ],
      fiveYO:
    ...
    /* ---------------------------------------------------------- 4-3 */
    {
      ...
      diagram: "matrix",
      deep: [
        { h: "學測陷阱：乘法不可交換", b: "矩陣乘法 AB≠BA(順序不可交換)，且要列數對行數才可乘。行列式=0 代表沒有唯一解(反矩陣不存在)，別硬套反矩陣硬解。" },
        { h: "解題步驟：聯立寫成矩陣", b: "三元一次方程組寫成 A·x = b，再用反矩陣 x = A⁻¹·b 或克拉瑪公式(各變數＝替換行行列式 / 係數行列式)求解。" },
        { h: "技巧：2×2 反矩陣速算", b: "2×2 矩陣 [[a,b],[c,d]] 的反矩陣 = (1/det)·[[d,−b],[−c,a]]，其中 det=ad−bc。det=0 就無反矩陣。" }
      ],
      fiveYO:
