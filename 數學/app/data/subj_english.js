  ...
  chapters: [
    {
      ...
      deep: [
        { h: "學測陷阱：4500 不夠用", b: "課內考 4500 字，但實戰要以 7000 字打底。單字量不足會直接拖慢閱讀速度，連聽力理解都受影響——單字是英文所有題型的地基。" },
        { h: "解題步驟：拆字猜意思", b: "用字根字首拆字猜意(in-／un- 否定、re- 再次、-tion 名詞)；搭配詞(collocation)整組記，如 make a decision、pay attention to。" },
        { h: "技巧：SRS 間隔背誦", b: "用本 App 的間隔重複(SRS)每天背，答對拉長間隔、答錯隔天再轟炸，逼進長期記憶，比一次硬背有效得多。" }
      ],
    ...
    },
    {
      ...
      deep: [
        { h: "學測陷阱：時態一致與假設語氣", b: "時態一致與主動詞單複數一致最常考。假設語氣『與現在事實相反』要把動詞往後退一格(過去式、be 用 were)，是高頻易錯。" },
        { h: "解題步驟：主謂→時間→子句", b: "先看主詞動詞是否一致→由時間副詞定時態(since／for→完成式；ago→過去式)→關係子句看先行詞是人(who)還是物(which)。" },
        { h: "技巧：被動與假設兩型", b: "看到動作承受者當主詞或句中 by＋行為者多半是被動(be＋p.p.)。假設語氣分『與現在相反』與『與過去相反(had＋p.p., would have)』兩型。" }
      ],
    ...
    },
    {
      ...
      deep: [
        { h: "學測陷阱：克漏字考三件事", b: "克漏字常同時考『連接詞邏輯＋搭配詞＋時態詞性』。閱讀題的『過度推論』與『太絕對(all／always／never)』選項是兩大坑。" },
        { h: "解題步驟：先看題再定位", b: "閱讀先掃題目關鍵字→回原文定位該段→鎖定句子作答，不要從頭讀到尾。克漏字先看空格前後的邏輯關係(轉折／因果／並列)再選。" },
        { h: "技巧：同義改寫常是答案", b: "正解多為原文的『同義改寫』；與原文一字不差照抄的選項反而常是陷阱。猜生字用語境(對比、舉例 such as)推測。" }
      ],
    ...
    },
    {
      ...
      deep: [
        { h: "學測陷阱：結構鬆散與文法錯", b: "作文失分主因是結構鬆散、文法錯多、未扣題。寧可句子簡單但正確，也不要複雜卻出錯——文法錯最扣分。" },
        { h: "解題步驟：先列大綱再下筆", b: "動筆前花 2 分鐘列四段大綱(開頭破題→例證→例證→收束)，用連接詞(firstly／however／as a result)銜接，方向才不會跑。" },
        { h: "技巧：句型多變但求穩", b: "適度用關係子句、分詞構句、not only…but also 增加變化；看圖作文先客觀描述再加評論，結尾呼應開頭給完整感。" }
      ],
    ...
    },
    {
      ...
      deep: [
        { h: "學測陷阱：字面意思常被騙", b: "片語動詞合起來意思常與字面差很多(give up=放棄、put off=延後)；固定介系詞搭配不能換(depend on、interested in)，是高頻考點。" },
        { h: "解題步驟：當一個單字記", b: "把『動詞＋介副詞』當一個單字記語意；空格只有介系詞選項時回想動詞的固定搭配(consist of、result from)，再用句意確認方向。" },
        { h: "技巧：可分片語的代名詞", b: "可分片語遇代名詞當受詞要夾中間(turn it off、look it up)。result in 接『結果』、result from 接『原因』，方向相反勿混。" }
      ],
    ...
    },
    {
      ...
      deep: [
        { h: "學測陷阱：although 與 but 不可同句", b: "連接詞題考前後邏輯關係(轉折／因果／並列／對比)。中式英文最常錯把 although 與 but 放同一句並用——兩者二擇一。" },
        { h: "解題步驟：判邏輯再選詞", b: "先讀前後兩句判斷邏輯關係→選對應連接詞→檢查詞性接法：although／because 接子句；however／therefore 是副詞要加分號；despite 接名詞。" },
        { h: "技巧：篇章靠指示語", b: "段落間用 first／in addition／in conclusion 引導；代名詞與指示語(this／that／such)順著指涉就能掌握篇章邏輯。" }
      ],
    ...
    },
    {
      ...
      deep: [
        { h: "學測陷阱：逐字直譯", b: "中譯英不是逐字翻。中文無時態、英文要補上時態、單複數與冠詞；『很有興趣』是 be interested in 而非 have interest very。" },
        { h: "解題步驟：先抓主幹", b: "先找中文句的主詞、動詞、受詞立骨架→套對英文句型與時態→再掛上時間、地點、原因等修飾語；無主詞句用 It is said that 或被動處理。" },
        { h: "技巧：繞道用會的字", b: "難詞不會寫時用會的字改寫(paraphrase)表達同義；先求文法正確、句子通順，再求用字精準，對而通順勝過華麗卻出錯。" }
      ],
